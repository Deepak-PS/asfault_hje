...
class TestSuiteEvaluation:
    ...
    def __init__(self, result, score, oob, hje, timeouts, coverage, total_coverage, duration):
        self.result = result
        self.score = score
        self.oob = oob
        self.hje = hje
...
class SuiteEvaluator:
    def evaluate_suite(self, suite):
        ...
        ret = TestSuiteEvaluation(TestSuiteEvaluation.RESULT_SUCCESS, sum, oob, hje, timeouts, coverage, total_coverage, eval_dur)
        l.info(ret)
        return ret
...
class BeamNGEvaluator(SuiteEvaluator):
    ...
    def evaluate_test(self, test, suite):
        ...
        # if execution.average_acceleration:
        #  return execution.maximum_jerk, execution.hjes
        else:
            return -1, -1
...
class MaxJerkEvaluator(SuiteEvaluator):
    def evaluate_test(self, test, suite):
        if test.execution.result == RESULT_SUCCESS:
            fitness = test.execution.maximum_jerk
        else:
            fitness = -1

        return fitness, test.execution.reason

class LaneJerkEvaluatorWithWeights(SuiteEvaluator):
    def evaluate_test(self, test, suite):
        if test.execution.result == RESULT_SUCCESS:
            fLane = test.execution.maximum_distance / c.ev.lane_width
            fLane = min(fLane, 1.0)

            fJrk = test.execution.maximum_jerk

            fitness = 0.75 * fLane + 0.25 * fJrk
        else:
            fitness = -1

        return fitness, test.execution.reason

class LaneJerkEvaluator(SuiteEvaluator):
    def evaluate_test(self, test, suite):
        if test.execution.result == RESULT_SUCCESS:
            if test.execution.maximum_jerk > 0:
                fitness = test.execution.maximum_jerk
            else:
                fLane = test.execution.maximum_distance / c.ev.lane_width
                fitness = min(fLane, 1.0)
